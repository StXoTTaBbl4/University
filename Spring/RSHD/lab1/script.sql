...
DECLARE
    ...
    table_name text;
BEGIN
    -- Проверяем наличие схемы в fullname
    IF strpos(fullname, '.') > 0 THEN
        -- Если есть схема (формат schema.table)
        table_schema := split_part(fullname, '.', 1);
        table_name := split_part(fullname, '.', 2);
    ELSE
        -- Если схемы нет, используем search_path
        table_schema := current_schema();
        table_name := fullname;
    END IF;

    -- Проверяем существование таблицы в указанной схеме
    SELECT EXISTS (
        SELECT 1
    ...
    IF table_exists THEN
        RAISE NOTICE 'Информация о таблице: %.%', table_schema, table_name;
        ...
        FOR rec IN
            ...
            WHERE
                a.attrelid = (table_schema || '.' || table_name)::regclass
                AND a.attnum > 0
        ...
        LOOP
            ...
            WHERE objoid = (table_schema || '.' || table_name)::regclass
              AND objsubid = (SELECT a.attnum FROM pg_catalog.pg_attribute a
                              WHERE a.attrelid = (table_schema || '.' || table_name)::regclass AND a.attname = rec.attname);
            ...
            WHERE t.oid = (table_schema || '.' || table_name)::regclass;
        ...
        END LOOP;
    ELSE
        RAISE NOTICE 'Таблица %.% не существует или у вас нет к ней доступа', table_schema, table_name;
    END IF;
END;
$$;
