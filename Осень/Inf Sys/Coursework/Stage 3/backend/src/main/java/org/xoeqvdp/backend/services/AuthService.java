...
public class AuthService {
    ...
    public String refreshOrUpdateRefreshToken(String email, String password) {
        Employee employee = employeeRepository.findByEmail(email).orElse(null);
        ...
        if (refreshToken != null && refreshToken.getExpiresAt().isAfter(Instant.now()) && !refreshToken.isRevoked()) {
    ...
    public boolean validateRefreshToken(String token){
        RefreshToken refreshToken = refreshTokenRepository.findByEmployee_Email(jwtService.extractEmail(token)).orElse(null);

        if (refreshToken != null && refreshToken.getExpiresAt().isAfter(Instant.now())) {
            return true;
        } else {
            throw new RuntimeException("refreshToken отсутствует или просрочен.");
        }
    }
